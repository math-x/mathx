...
namespace mathx {
     ...
     */
    inline void int_l::null() {
        num = "";
    }

    inline void int_l::remove_trailing_zeroes() {
        int l = 0;
        while(num[l] == '0')
            l++;
        num = num.substr(l);
    ...
    inline int_l::int_l (std::string x) {
        if(x[0] == '-') {
            is_positive = false;
            num = x.substr(1);
        }
    ...
    inline int_l int_l::operator+ (const int_l &y) {
        int_l res = y;
    ...
    inline int_l int_l::operator+ (const INT &y) {
        int_l x = y;
        return (*this) + x;
    }

    inline int_l operator+ (const INT &x, int_l &y) {
        return y + x;
    }

    inline int_l int_l::operator- (const int_l &z) {
        int_l x = *(this);
        int_l y = z;
    ...
    inline std::ostream& operator<<(std::ostream &os, const int_l &c) {
        if (!c.is_positive)
            os << '-';
        os << c.num;
        return os;
    ...
    inline void int_l::operator= (const INT &x) {
        if (x >= 0) {
            is_positive = true;
    ...
    inline void int_l::operator= (const std::string &x) {
        if(x[0] == '-') {
            is_positive = false;
            num = (x.substr(1));
        }
    ...
    inline bool int_l::operator== (const int_l &x) {
        if(is_positive == x.is_positive && num == x.num)
            return true;
        return false;
    }

    inline bool int_l::operator== (const INT &x) {
        if(is_positive == (x >= 0)) {
            if(x >= 0)
    ...
    inline int_l int_l::operator++ (int x) {
        int_l y = *this;
        *this = *this + 1;
        return y;
    }
